...
| Item | Details |
|------|---------|
| Endpoint | `GET https://app-dm6bkvkknf29-api-oYA6Z8wDBRDa.gateway.appmedo.com/v1/videos/image2video/{id}` |
...
The script polls the task until it succeeds or fails, downloads the resulting video (upstream CDN links are time-limited, so it downloads immediately), and prints one JSON line to stdout:
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dm6bkvkknf29-api-oYA6Z8wDBRDa.gateway.appmedo.com/v1/videos/image2video/${encodeURIComponent(taskId)}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
