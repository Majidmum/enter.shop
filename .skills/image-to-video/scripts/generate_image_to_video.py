...
SUBMIT_URL = "https://app-dm6bkvkknf29-api-rY7JZvg0dqdL.gateway.appmedo.com/v1/videos/image2video"
QUERY_URL_BASE = "https://app-dm6bkvkknf29-api-oYA6Z8wDBRDa.gateway.appmedo.com/v1/videos/image2video"
POLL_INTERVAL_S = 7
SAFE_LIMIT_S = 550
...
def file_to_base64(path):
    """Read a local file and encode it as a pure Base64 string (no data: prefix)."""
...
def parse_args():
    ...
    p = argparse.ArgumentParser(
        description="Submit a Kling image-to-video task and poll for the result."
    )
    group = p.add_mutually_exclusive_group(required=True)
    group.add_argument("--image", help="local path of the reference image (sent as pure Base64)")
    group.add_argument("--image-url", dest="image_url", help="URL of the reference image")
    p.add_argument("--image-tail", dest="image_tail", help="tail frame image URL or pure Base64 string")
    p.add_argument("--prompt", help="text prompt describing the desired motion/content")
    p.add_argument("--negative-prompt", dest="negative_prompt", help="content to avoid in the video")
    p.add_argument("--model", default="kling-v1-6", help="model name (default: kling-v1-6)")
    p.add_argument("--mode", choices=["std", "pro"], help="generation mode: std or pro")
    p.add_argument("--duration", choices=["5", "10"], default="5", help="video duration in seconds")
    p.add_argument("--aspect-ratio", dest="aspect_ratio", help="aspect ratio, e.g. 16:9, 9:16, 1:1")
    p.add_argument("--cfg-scale", dest="cfg_scale", type=float, help="prompt adherence [0, 1]")
    p.add_argument("--camera-control", dest="camera_control",
                   help="camera control config as a JSON string")
    p.add_argument("--output-dir", dest="output_dir", help="directory to download result videos into")
    args = p.parse_args()

    if args.cfg_scale is not None and not 0 <= args.cfg_scale <= 1:
        p.error("--cfg-scale must be in range [0, 1]")
...
def submit_task(args, key):
    """Submit the image-to-video task and return the task id."""
    payload = {
    ...
    if args.image_tail:
        payload["image_tail"] = args.image_tail
    if args.prompt:
    ...
    if args.camera_control:
        try:
            payload["camera_control"] = json.loads(args.camera_control)
        except json.JSONDecodeError as e:
            raise RuntimeError("--camera-control must be valid JSON: %s" % e)
...
def main():
    """Entry point: submit and poll the image-to-video task, then print the result JSON."""
    args = parse_args()
    key = api_key()
    try:
