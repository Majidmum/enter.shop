#!/usr/bin/env python3
"""
Poll an already-submitted Kling omni-video task by task_id (does NOT submit a new task).

Use this when generate_omni_video.py returned {"status": "processing", "task_id": "..."}.
...
QUERY_URL_BASE = "https://app-dm6bkvkknf29-api-pLVzAEz1ZQOL.gateway.appmedo.com/v1/videos/omni-video"
POLL_INTERVAL_S = 7
SAFE_LIMIT_S = 550
...
def main():
    """Entry point: poll an existing omni-video task and print the result JSON."""
    parser = argparse.ArgumentParser(description="Poll a Kling omni-video task.")
    parser.add_argument("--task-id", required=True, dest="task_id", help="task ID to poll")
    parser.add_argument("--output-dir", dest="output_dir", help="output directory for result videos")
